...
const ReductionForm = ({get_response, update_plugin, ...props}) => {

    const [config_visible, set_config_visible] = useState(false);
    const [values, set_values] = useState(null);

    const start_config = () => {
        set_values(get_response.reduction_values);
        set_config_visible(true);
    };

    const submit_config = () => {
        update_plugin({
            reduction_values: values
        });
        set_config_visible(false);
    };

    return (
        <div className={"reduction-form"} {...props}>

            <Section
                boxed
                title={"reduction configuration"}
            ...
            >
                {config_visible
                    ? (
                        <AutoForm
                            form={get_response?.reduction_form}
                            values={get_response?.reduction_values}
